...
class TijiantwoController extends CommonController {
	   public function lists(){
		...
		if($_REQUEST["Quanxianname"]){
			...
			$show       = $Page->show();// 分页显示输出
			// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
			$list = $Quanxian->where("keshi = '体检中心'")->select();
			//当前数据
			$this -> assign("listsa",$list);
	...
	public function del(){
		...
		$info = M("tjmingxi")->where("id = $id")->delete();
	...
	public function edit(){
		...
		$info = M("tjmingxi")->where("id = $id")->find();
	...
	public function editAction(){
		...
		$info = M("tjmingxi")->where("id = $id")->save($_POST);
